  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      const res = await pb.collection('locations').getList<Location>(1, 10, {
        filter: `name ~ "${locationQuery}"`, sort: 'name', requestKey: null,
      })
  ...
  const save = async () => {
    ...
    try {
      ...
      if (selectedLocation && Number(qty) > 0) {
        await pb.collection('inventory').create({ part_id: part.id, location_id: selectedLocation.id, qty: Number(qty) })
        await pb.collection('transactions').create({ type: 'incoming', part_id: part.id, location_id: selectedLocation.id, qty: Number(qty) })
      }
      router.push('/parts')
  ...
  return (
    <div className="p-4 space-y-4 max-w-2xl mx-auto">
      <div className="flex items-center gap-3 pt-4">
        ...
        <h1 className="text-xl font-bold">Нова деталь</h1>
      </div>
      ...
      <div className="border rounded-xl p-4 space-y-3">
        ...
        <FIELD label="Назва *"><Input placeholder="Резистор 10кОм" value={form.name} onChange={set('name')} /></FIELD>
        <FIELD label="Тип деталі"><Input placeholder="Резистор / Конденсатор / Блок ABS..." value={form.part_type} onChange={set('part_type')} /></FIELD>
        ...
        <FIELD label="Артикул"><Input placeholder="R-10K-0805" value={form.article} onChange={set('article')} /></FIELD>
      </div>
      ...
      <div className="border rounded-xl p-4 space-y-3">
        ...
        <FIELD label="Виробник"><Input placeholder="Vishay, Samsung..." value={form.manufacturer} onChange={set('manufacturer')} /></FIELD>
        <FIELD label="Штрихкод (EAN)"><Input placeholder="4901234567890" value={form.barcode} onChange={set('barcode')} /></FIELD>
        <FIELD label="Datasheet URL"><Input placeholder="https://..." value={form.datasheet_url} onChange={set('datasheet_url')} /></FIELD>
      </div>

      {/* Розміщення */}
      <div className="border rounded-xl p-4 space-y-3">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide flex items-center gap-1"><MapPin size={12} />Розміщення</p>
        {selectedLocation ? (
          ...
          <div className="relative">
            ...
            <Input className="pl-8" placeholder="Назва комірки (A-1-1)..." value={locationQuery}
                ...
                {locationQuery.trim() && !locations.find(l => l.name === locationQuery.trim()) && (
                  <button className="w-full text-left px-4 py-2.5 text-sm text-primary hover:bg-accent transition-colors border-t"
                    onClick={async () => {
                      ...
                      setSelectedLocation(newLoc as Location)
                      setShowLocations(false)
                      setLocationQuery('')
                    }}>
                    + Створити комірку «{locationQuery.trim()}»
                  </button>
                )}
              </div>
            )}
        ...
        <FIELD label="Початкова кількість">
          <Input type="number" min="0" value={qty} onChange={e => setQty(e.target.value)} className="text-center font-bold" />
        </FIELD>
      </div>
      ...
      <div className="border rounded-xl p-4 space-y-3">
        ...
        <FIELD label="Опис"><Textarea placeholder="Опис деталі..." value={form.description} onChange={set('description')} /></FIELD>
        <FIELD label="Нотатки"><Textarea placeholder="Нотатки..." value={form.notes} onChange={set('notes')} /></FIELD>
      </div>

      <Button className="w-full h-12 text-base" onClick={save} disabled={saving || !form.name.trim()}>
        {saving ? 'Збереження...' : 'Зберегти деталь'}
      </Button>
    </div>
  )
}
