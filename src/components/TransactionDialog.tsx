    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-4">
          ...
          <div>
            <p className="text-sm font-medium mb-2">{isIncoming ? 'Кількість (приход)' : 'Кількість (витрата)'}</p>
            <div className="flex items-center gap-3">
              <Button type="button" variant="outline" size="icon" className="h-12 w-12 text-xl shrink-0"
                onClick={() => { const n = Math.max(1, Number(qty) - 1); setQty(String(n)); setError('') }}>
                <Minus size={18} />
              </Button>
              <div className="flex-1 text-center text-3xl font-bold py-2 border rounded-xl bg-accent">
                {qty}
              </div>
              <Button type="button" variant="outline" size="icon" className="h-12 w-12 text-xl shrink-0"
            ...
            {type === 'outgoing' && (
              <p className="text-xs text-muted-foreground mt-2 text-center">Залишиться: {Math.max(0, currentQty - Number(qty))} шт</p>
            )}
            {type === 'incoming' && (
              <p className="text-xs text-muted-foreground mt-2 text-center">Буде: {currentQty + Number(qty)} шт</p>
            )}
          </div>

          <div>
            <p className="text-sm font-medium mb-1">Нотатка (необов'язково)</p>
